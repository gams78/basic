<?php
/**
 * Message translations.
 *
 * This file is automatically generated by 'yii message' command.
 * It contains the localizable messages extracted from source code.
 * You may modify this file by translating the extracted messages.
 *
 * Each array element represents the translation (value) of a message (key).
 * If the value is empty, the message is considered as not translated.
 * Messages that no longer need translation will have their translations
 * enclosed between a pair of '@@' marks.
 *
 * Message string can be used with plural forms format. Check i18n section
 * of the guide for details.
 *
 * NOTE: this file must be saved in UTF-8 encoding.
 */
return [
    'A message has been sent to your email address. It contains a confirmation link that you must click to complete registration.' => '',
    'A new confirmation link has been sent' => '',
    'A password will be generated automatically if not provided' => '',
    'An email has been sent with instructions for resetting your password' => '',
    'An error occurred and your password has not been changed. Please try again later.' => '',
    'An error occurred processing your request' => '',
    'Are you sure you want to block this user?' => '',
    'Are you sure you want to confirm this user?' => '',
    'Are you sure you want to delete this user?' => '',
    'Are you sure you want to unblock this user?' => '',
    'Are you sure? Deleted user can not be restored' => '',
    'Awesome, almost there. Now you need to click the confirmation link sent to your new email address' => '',
    'Awesome, almost there. Now you need to click the confirmation link sent to your old email address' => '',
    'Bio' => '',
    'Block' => '',
    'Block status' => '',
    'Blocked at {0, date, MMMM dd, YYYY HH:mm}' => '',
    'Change your avatar at Gravatar.com' => '',
    'Complete password reset on {0}' => '',
    'Confirm account on {0}' => '',
    'Confirm email change on {0}' => '',
    'Confirmation' => '',
    'Confirmation status' => '',
    'Confirmation time' => '',
    'Confirmed' => '',
    'Confirmed at {0, date, MMMM dd, YYYY HH:mm}' => '',
    'Connect' => '',
    'Continue' => '',
    'Create' => '',
    'Create a user account' => '',
    'Credentials will be sent to the user by email' => '',
    'Current password' => '',
    'Current password is not valid' => '',
    'Didn\'t receive confirmation message?' => '',
    'Don\'t have an account? Sign up!' => '',
    'Email (public)' => '',
    'Error occurred while changing password' => '',
    'Error occurred while confirming user' => '',
    'Error occurred while deleting user' => '',
    'Gravatar email' => '',
    'If you already registered, sign in and connect this account on settings page' => '',
    'If you cannot click the link, please try pasting the text into your browser' => '',
    'If you did not make this request you can ignore this email' => '',
    'In order to complete your registration, please click the link below' => '',
    'In order to complete your request, please click the link below' => '',
    'In order to finish your registration, we need you to enter following fields' => '',
    'Invalid or expired link' => '',
    'Joined on {0, date}' => '',
    'New email' => '',
    'New password' => '',
    'New permission' => '',
    'Not blocked' => '',
    'Please click the link below to complete your password reset' => '',
    'Please fix following errors:' => '',
    'Profile details have been updated' => '',
    'Recover your password' => '',
    'Recovery link is invalid or expired. Please try requesting a new one.' => '',
    'Recovery message sent' => '',
    'Registration IP' => '',
    'Registration ip' => '',
    'Registration time' => '',
    'Remember me next time' => '',
    'Request new confirmation message' => '',
    'Reset your password' => '',
    'Sign In' => '',
    'Sign Up' => '',
    'Something went wrong' => '',
    'Something went wrong and your account has not been confirmed.' => '',
    'Thank you for signing up on {0}' => '',
    'Thank you, registration is now complete.' => '',
    'The confirmation link is invalid or expired. Please try requesting a new one.' => '',
    'There is no user with this email address' => '',
    'This account has already been confirmed' => '',
    'This account has already been connected to another user' => '',
    'This email address has already been taken' => '',
    'This username has already been taken' => '',
    'Unblock' => '',
    'Unconfirmed' => '',
    'Update' => '',
    'Update user account' => '',
    'User has been blocked' => '',
    'User has been confirmed' => '',
    'User has been created' => '',
    'User has been deleted' => '',
    'User has been unblocked' => '',
    'User is not found' => '',
    'VKontakte' => '',
    'We have generated a password for you' => '',
    'We have received a request to change the email address for your account on {0}' => '',
    'We have received a request to reset the password for your account on {0}' => '',
    'We have sent confirmation links to both old and new email addresses. You must click both links to complete your request' => '',
    'Website' => 'Spletišče',
    'Welcome to {0}' => 'Dobrodošel v {0}',
    'Yandex' => '',
    'You can assign multiple roles or permissions to user by using the form below' => 'Spodaj lahko dodeliš več vlog in dovoljenj',
    'You can connect multiple accounts to be able to log in using them' => 'Ne moreš se povezati z večimi računi',
    'You can not block your own account' => 'Ne moreš blokirati svojega računa',
    'You can not remove your own account' => 'Ne moreš odstraniti svojega računa',
    'You need to confirm your email address' => 'Potrditi moraš tvoj elektronski naslov',
    'Your account details have been updated' => 'Podrobnosti tvojega računa so posodobljene',
    'Your account has been blocked' => 'Tvoj račun je blokiran',
    'Your account has been blocked.' => 'Tvoj račun je blokiran.',
    'Your account has been connected' => 'Tvoj račun je povezan',
    'Your account has been created' => 'Tvoj račun je ustvarjen',
    'Your account has been created and a message with further instructions has been sent to your email' => '',
    'Your account on {0} has been created' => 'Tvoj uporabniški račun pri {0} je bil ustvarjen',
    'Your confirmation token is invalid or expired' => 'Tvoj potrditveni žeton je napačen ali je že potekel',
    'Your email address has been changed' => 'Tvoj elektronski naslov je bil spremenjen',
    'Your password has been changed successfully.' => 'Tvoje geslo je bilo uspešno spremenjeno.',
    'Your profile has been updated' => 'Tvoj profil je bil posodobljen',
    '{0, date, MMMM dd, YYYY HH:mm}' => '{0, date, MMMM dd, YYYY HH:mm}',
    '(not set)' => '(ni nastavljeno)',
    'A confirmation message has been sent to your new email address' => 'Potrditveno sporočilo je bilo poslano na tvoj elektronski naslov',
    'Account' => 'Račun',
    'Account confirmation' => 'Potrditev računa',
    'Account details' => 'Podrobnosti računa',
    'Account details have been updated' => 'Podrobnosti računa so bile posodobljene',
    'Account settings' => 'Nastavitve računa',
    'Already registered? Sign in!' => 'Si že registriran/-a? Prijavi se!',
    'Assignments' => 'Vloge',
    'Confirm' => 'Potrdi',
    'Delete' => 'Izbriši',
    'Disconnect' => 'Prekini',
    'Email' => 'Elektronski naslov',
    'Finish' => 'Končaj',
    'Forgot password?' => 'Pozabljeno geslo?',
    'Hello' => 'Zdravo',
    'Information' => 'Informacije',
    'Invalid login or password' => 'Napačno uporabniško ime ali geslo',
    'Location' => 'Lokacija',
    'Login' => 'Vpis',
    'Logout' => 'Izpis',
    'Manage users' => 'Upravljaj uporabnike',
    'Name' => 'Ime',
    'Networks' => 'Omrežja',
    'New role' => 'Nova vloga',
    'New user' => 'Nov uporabnik',
    'Password' => 'Geslo',
    'Password has been changed' => 'Geslo je bilo spremenjeno',
    'Permissions' => 'Dovoljenja',
    'Profile' => 'Profil',
    'Profile details' => 'Podrobnosti profila',
    'Profile settings' => 'Nastavitve profila',
    'Roles' => 'Vloge',
    'Save' => 'Shrani',
    'Sign in' => 'Vpis',
    'Sign up' => 'Prijava',
    'Username' => 'Uporabniško ime',
    'Users' => 'Uporabniki',
];
